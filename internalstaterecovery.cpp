...
void InternalStateRecovery::run()
{
    ...
    PosInt resumeClock = 0;//Exclusive to Fix 3 combined with Fix 1
    struct Stats
    {
        PosInt sieveIndex = 0;
    ...
    PosInt totalTime = 0;
    #endif
    PosInt numOfGCAboveOneHalf = 0;//NEW!
    const Real average_guess_capacity = CalculationUtility::calculateAverageGC(lfsr, key, numOfGCAboveOneHalf);
    const Real speed_factor = CalculationUtility::calculateSpeedFactor( stateBits.size(), numOfGCAboveOneHalf, C_LFSR_SIZE );
    const PosInt d = 0;
    ...
    const PosInt max_clocks = 1000; //NEW!!

    GeneratedStatesType generatedKeystream = KeystreamGenerator::generate(INITIAL_LFSR_STATE, INITIAL_KEY, max_clocks);//NEW! CHANGED!
    ...
    auto algorithmCore = [ &lfsr, &key, &clock_offset, &generatedKeys, &generatedKeystream, &output_capacity_theta, &alpha_threshold, &totalClocks
        #if defined(CLOCK_DEBUG)
            ...
            , &resumeClock
        #endif
            ] (QVector<StateWithMismatchCounter*>& CUR, const PosInt& alpha_terminate) {

        QVector<StateWithMismatchCounter*> NEW;
        QVector<StateWithMismatchCounter*> STORED;
        ...
        for( PosInt i = 0; i < clock_offset; i++)
        {
             ...
             lfsr.shiftLeft(fb);
             key.rotateLeft();
             //qDebug() << "lel" << "i" << i << "clock_offset" << clock_offset;
        }
        //Experimental

        const PosInt firstClock = first_clock_t + clock_offset;

        for( PosInt clock = firstClock; clock < max_clocks; clock++)
        {
            //qDebug() << "***************************************************************************";
            ...
            for ( StateWithMismatchCounter* candidateState: CUR)
            {
                ...
                //NEW! Check for exit condition

                if ( candidateState->getSuggestedValueCount() >= alpha_terminate ){
                    #if defined(SIEVE_MODE)
                    candidateState->setStoredClock(clock);
                    if (resumeClock == 0)
                    {
                        resumeClock = clock;
                        //qDebug() << "Resume Clock is set to" << resumeClock;
                    }
                    else
                    {
                        //Nothing
                        ...
                        candidateState->setMismatchValue(0);//NEW!!!
                    #endif
                    #endif
                    STORED.append(candidateState);
                    continue;
                }


                #if defined(SIEVE_MODE)
                // Check wheter resume condition met
                if (candidateState->getStoredClock() > clock)
                {
                    //Wait for next clock without doing nothing
                    NEW.push_back(candidateState);
                    continue;
                }
                #endif
                ...
                if ( gcf == 0.5 )
                {
                    ...
                    //Skip further tests for the state, and clock it
//                    lfsr.shiftLeft(currentFeedback);
//                    candidateState->loadRegisterValue(lfsr.registerValue());
//                    NEW.push_back(candidateState);
//                    totalClocks++;
//                    continue;
                }
                else
                {
                    ...
                    candidateState->incrementSuggestedValueCount();//NEW!
                }
    ...
    //FIX 3
    //Real divided_alpha_terminate = alpha_terminate / (alpha_terminate / 30);
    ...
    for ( PosInt stateIndex = startIndex; stateIndex < endIndex; stateIndex += candidateBufferSize  )
    {
        ...
        QVector<StateWithMismatchCounter*> aliveStates;
        #if defined ( SIEVE_MODE)
                ...
                aliveStates = algorithmCore(candidates, globalAlphaTerminate);
        #else
                candidates = fetchLimitedCandidates(candidateArrays, startIndex, candidateBufferSize, C_LFSR_SIZE);
                aliveStates = algorithmCore(candidates, alpha_terminate);
        #endif
    ...
    #else
    if (!hitStates.isEmpty())
    {
        qDebug() << "Reducing Results: ";
        QVector<StateWithMismatchCounter*> reducedList = reduceDuplicateRootCandidates(hitStates);
        qDebug() << "Printing Reduced List:";
        printStates(reducedList);
    ...
    bool still_alive = false;
     ...
     if (state->suggestedValueExists() && state->getSuggestedValue() != determinedFeedback)
     {
        ...
        still_alive = true;
         ...
         {
             #if defined (TRACK_CORRECT_STATE)
             if ( state->getRootStateIndex() == C_INITIAL_STATE_DECIMAL )
             {
                 state->printRegister();
                 qDebug() << "Bye Bye (Threshold'dan Gitti)";
             }
             #endif
             still_alive = false;
             delete state;
...
bool InternalStateRecovery::check_and_guess( StateWithMismatchCounter* state, QVector<StateWithMismatchCounter *> &NEW, const Real &alpha_threshold, const Bit &currentOutput, const Bit &actualOutput)
{
    bool forked = false;
    #ifdef VERBOSE
        qDebug() << "Check & Guess";
    ...
    else
    {
        ...
        if ( state->getRootStateIndex() == C_INITIAL_STATE_DECIMAL )
        {
            ...
            state->resetToOrigin();
            state->printRegister();
            qDebug() << "Bye Bye (Output'tan Gitti)";
        }
        #endif
        delete state;
        #ifdef VERBOSE
