...
Real CalculationUtility::calculateAverageGC(const LinearFeedbackShiftRegister &p_lfsr, const KeyRegister &p_key, PosInt& p_numOfCalculations)
{
    ...
    p_numOfCalculations = numOfCalculations;
    return avgGC;
}
...
Real CalculationUtility::calculateSpeedFactor(const PosInt &numOfPossibleStates, const PosInt &numOfGCAboveOneHalf, const PosInt &lfsrSize)
{
    Real b = pow(2, lfsrSize - numOfPossibleStates);
    Real a = b * numOfGCAboveOneHalf;
    Real r = a / pow(2, lfsrSize);
    #if defined(CALCULATIONS_DEBUG)
        qDebug() << "Speed Factor:";
        qDebug() << QString("Speed Factor(r) = a / 2^s");
        qDebug() << QString().sprintf("%f = %f / %f", r, a, pow(2, lfsrSize));
    #endif
    return r;
}
